...
export const fetchWadaslintangLoadValue = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TCP.WD01.Real_Power_TOTAL' }),
    });
...
export const fetchWadaslintangLoadSecondValue = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TCP.WD02.Real_Power' }),
    });
