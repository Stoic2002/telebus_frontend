...
export const fetchTulisLoadValue = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TCP.TL01.tls_u1_gen_active_power' }),
    });
...
export const fetchTulisLoadSecondValue = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      method: 'POST',
      headers: {
