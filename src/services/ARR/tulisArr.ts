...
export const fetchTulisArrPerDay = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TELEMETERING.ARR.ARR_ST02_DAY' }),
    });
...
export const fetchTulisArrPerSec = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL_OPC}/readOPCTag`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TELEMETERING.ARR.ARR_ST02_RT' }),
    });
