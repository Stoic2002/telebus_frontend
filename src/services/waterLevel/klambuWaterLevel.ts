...
export const fetchKlambuWaterLevelAo = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/readOPCTag`, {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TCP.KL01.KLB_AO_WATER_LEVEL_DISPLAY' }),
    });
...
export const klambuWaterLevelHmi = async (): Promise<SensorValueResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/readOPCTag`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ nodeId: 'ns=2;s=MRC_TCP.KL01.KLB_HMI_WATER_LEVEL_VN91' }),
    });
