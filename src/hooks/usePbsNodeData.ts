...
export const usePbsNodeData = ({ interval = 30000 }: UseNodeDataProps = {}): UseNodeDataReturn => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await pbsService.getAllNodes();
        setData(response);
        setError(null);

        const soedirman = {
          activeLoads: {
            pb01: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB01.ACTIVE_LOAD", response),
            pb02: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB02.ACTIVE_LOAD", response),
            pb03: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB03.ACTIVE_LOAD", response),
            total: 0 // Placeholder
          },
          flows: {
            inflow: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00.inflow_waduk", response),
            turbine: {
              pb01: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB01.flow_turbin", response),
              pb02: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB02.flow_turbin", response),
              pb03: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB03.flow_turbin", response),
              total: 0 // Placeholder
            },
            ddc: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00.flow_ddc", response),
            spillway: {
              g1: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.FLOW_SPW_G1", response),
              g2: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.FLOW_SPW_G2", response),
              g3: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.FLOW_SPW_G3", response),
              g4: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.FLOW_SPW_G4", response),
              total: 0 // Placeholder
            }
          },
          levels: {
            sediment: pbsService.getNodeValueById("ns=2;s=MRC_TELEMETERING.ARR.Level_Sedimen", response),
            elevation: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.ELV", response),
            waterDepth: pbsService.getNodeValueById('ns=2;s=MRC_TELEMETERING.ARR1.water_depth', response),
            waterDepthCalc: pbsService.getNodeValueById('ns=2;s=MRC_TELEMETERING.ARR.Water_Depth_Calc', response)
          },
          additionalData: {
            inflow1h: pbsService.getNodeValueById("ns=2;s=MRC_TCP.PB00SW.INFLOW_1H", response)
          }
        };
