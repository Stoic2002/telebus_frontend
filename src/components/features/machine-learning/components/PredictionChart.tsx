...
import { PredictionParameter, Y_AXIS_DOMAIN } from '@/types/machineLearningTypes';
...
export const PredictionChart: React.FC<PredictionChartProps> = ({ 
  ...
  if (loading) {
    return <ShimmerPage hasMetrics={false} hasCharts={true} hasTables={false} />;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="text-xl font-bold text-gray-900">
            {stats.average.toFixed(2)} {selectedParameter === 'TMA' ? 'm' : selectedParameter === 'BEBAN' ? 'MW' : 'm³/s'}
          </div>
        </div>
        <div className="bg-gray-50 p-4 rounded-lg">
          <div className="text-sm font-medium text-gray-600">Total Kemarin</div>
          <div className="text-xl font-bold text-gray-900">
            {stats.total.toFixed(2)} {selectedParameter === 'TMA' ? 'm' : selectedParameter === 'BEBAN' ? 'MW' : 'm³/s'}
          </div>
        </div>
        <div className="bg-gray-50 p-4 rounded-lg">
      ...
      <div className="flex justify-end">
        ...
        >
          ...
          Download {selectedParameter} CSV
        </Button>
      </div>
          ...
          <LineChart data={chartData} margin={{ top: 40, right: 30, left: 20, bottom: 50 }}>
            ...
            <Tooltip 
              ...
              formatter={(value: any, name: string) => [
                value ? `${Number(value).toFixed(2)} ${selectedParameter === 'TMA' ? 'm' : selectedParameter === 'BEBAN' ? 'MW' : 'm³/s'}` : 'No data',
